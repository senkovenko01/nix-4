package ua.com.alevel.service.impl;

import org.apache.log4j.Logger;
import ua.com.alevel.dao.AuthorDao;
import ua.com.alevel.dao.impl.AuthorDaoImpl;
import ua.com.alevel.entity.Author;
import ua.com.alevel.service.AuthorService;
import ua.com.alevel.service.BookService;

import java.util.List;

public class AuthorServiceImpl implements AuthorService {

    private final AuthorDao authorDao = new AuthorDaoImpl();
    private final BookService bookDao= new BookServiceImpl();

    private final Logger log = Logger.getLogger(AuthorServiceImpl.class);


    @Override
    public void create(Author data) {
        log.info("Creating author");
        Author author = authorDao.findAuthorByName(data.getFirstName(), data.getLastName());
        if( author == null){
            authorDao.create(data);
            log.info("Created author InMemoryDB " + data);
        }
        else {
            log.error("Author already exists " + author);
            throw new RuntimeException("Author already exists " + author);
        }
    }

    @Override
    public Author read(int id) {
        log.info("Reading author, id=" + id);

        if(!exist(id)) {
            log.error("Author doesn't exist, id = " + id);
            throw new RuntimeException("Author doesn't exist");
        }
        log.info("Read author InMemoryDB " + authorDao.read(id));
        return authorDao.read(id);
    }

    @Override
    public List<Author> read() {
        log.info("Read all authors InMemoryDB");
        return authorDao.read();
    }


    @Override
    public void update(Author data) {
        log.info("Updating author, id=" + data.getId());
        if(!exist(data.getId())) {
            log.error("Author doesn't exist, id=" + data.getId());
            throw new RuntimeException("Author doesn't exist");
        }
        authorDao.update(data);
        log.info("Author was updated " + authorDao.read(data.getId()));
    }


    @Override
    public void delete(int id) {
        log.info("Deleting author, id=" + id);
        if(!exist(id)) {
            log.error("Author doesn't exist, id= " + id);
            throw new RuntimeException("author doesn't exist");
        }
        authorDao.delete(id);
        log.info("Author was deleted, id=" + id);
    }

    @Override
    public boolean exist(int id) {
        log.info("Check if author exists, id=" + id);
        return authorDao.exist(id);
    }


    @Override
    public List<Author> findAuthorsByBook(int id) {
        log.info("Read authors by book, id=" + id);
        if(!bookDao.exist(id)){
            log.error("Book doesn't exist, id=" + id);
            throw new RuntimeException("Book doesn't exist, id=" + id);
        }
        return authorDao.readAuthorsByBook(id);
    }

    @Override
    public Author findAuthorByName(String firstName, String lastName) {
        log.info("Find author by name: " + firstName + " " + lastName);
        return authorDao.findAuthorByName(firstName, lastName);
    }
}
